...
speed_test() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [[ -n "${dl_speed}" && -n "${up_speed}" && -n "${latency}" ]]; then
            printf "\033[0;26m%-25s\033[0;32m%-18s\033[0;31m%-20s\033[0;36m%-9s\033[0m\n" " ${nodeName}" "${dl_speed}" "${up_speed}" "${latency}"
        fi
    fi
}
...
speed() {
    ...
    speed_test '1536' 'STC, HK'
    speed_test '32155' 'Chinamobile, HK'
    speed_test '34555'  'HKIX, HK'
...
install_speedtest() {
    if [ ! -e "./speedtest-cli/speedtest" ]; then
        ...
        wget --no-check-certificate -q -T3 -O speedtest.tgz ${url1}
        if [ $? -ne 0 ]; then
            wget --no-check-certificate -q -T3 -O speedtest.tgz ${url2}
